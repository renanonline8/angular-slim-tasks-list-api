...
abstract class Users implements ActiveRecordInterface
{
    ...
    /**
     * The value for the id_ext field.
     *
     * @var        string
     */
    protected $id_ext;
    ...
    /**
     * Set the value of [id_ext] column.
     *
     * @param string $v new value
     * @return $this|\Users The current object (for fluent API support)
     */
    public function setIdExt($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }
        ...
        if ($this->id_ext !== $v) {
            $this->id_ext = $v;
            $this->modifiedColumns[UsersTableMap::COL_ID_EXT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : UsersTableMap::translateFieldName('IdExt', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id_ext = (null !== $col) ? (string) $col : null;
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UsersTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UsersTableMap::COL_ID_EXT)) {
            $modifiedColumns[':p' . $index++]  = '`id_ext`';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case '`id`':
                        ...
                        break;
                    case '`id_ext`':
                        $stmt->bindValue($identifier, $this->id_ext, PDO::PARAM_STR);
                        break;
                    case '`email`':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UsersTableMap::COL_ID_EXT)) {
            $criteria->add(UsersTableMap::COL_ID_EXT, $this->id_ext);
        }
        if ($this->isColumnModified(UsersTableMap::COL_EMAIL)) {
    ...
    {
        $copyObj->setIdExt($this->getIdExt());
